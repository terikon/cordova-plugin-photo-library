...
@objc(PhotoLibraryProtocol) class PhotoLibraryProtocol : CDVURLProtocol {
                ...
                if url.host?.lowercased() == "thumbnail" {
                    ...
                    DispatchQueue.global(qos: .default).async {
                        service.getThumbnail(photoId!, thumbnailWidth: width!, thumbnailHeight: height!, quality: quality!) { (imageData) in
                            ...
                            let stream: NSInputStream = NSInputStream(data: imageData!.data)
                            self.sendResponseWithResponseCode(200, stream: stream, streamLength: imageData!.data.length, mimeType: imageData!.mimeType)
                        }
                    }
    ...
    // Variant that receives stream
    fileprivate func sendResponseWithResponseCode(_ statusCode: Int, stream: NSStream, streamLength: Int, mimeType: String?) {
        ...
        let response: CDVHTTPURLResponse = CDVHTTPURLResponse(url: self.request.url!, mimeType: mimeType, expectedContentLength: streamLength, textEncodingName: encodingName)
        response.statusCode = statusCode
